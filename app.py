...
def extract_form_name(page):
    """Grab the first non-empty line and strip trailing dots."""
...
def build_patterns(raw_input: str):
    """Comma-separated patterns; '*' → '.*' wildcard."""
    pats = []
...
remove_input = st.text_input("Remove patterns (* or regex, comma-separated)", "")
...
if st.button("Split & Download ZIP") and pdf_data:
    ...
    out.seek(0)
    st.download_button("Download all splits", out, file_name="acc_build_splits.zip")
