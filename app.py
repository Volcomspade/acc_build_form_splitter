...
def create_zip(pdf_bytes, patterns, prefix, suffix, remove_id):
    ...
    with zipfile.ZipFile(buf, 'w') as zf:
        for title, start, end in splits:
            # strip "#1234: " only for the filename
            file_title = re.sub(r'^#\s*\d+:\s*', '', title) if remove_id else title

            # apply regex/wildcard removals
            for rx in patterns:
...
with st.expander("ℹ️ Regex & wildcard tips"):
    ...
    - **Wildcard `*`**: matches any run of characters, e.g. `03.*_` removes `03.04_`, `03.03_`, etc.  
      Internally that becomes the regex `03\..*_`  
    - **Combine** with commas: `03.*_, L2_`  
    - You can also write a raw regex, e.g. `\d{2}\.\d{2}_` to remove any `xx.xx_` pattern.
    """)
...
if uploads:
    ...
    for idx, b in enumerate(all_bytes):
        ...
        for title, start, end in splits:
            # display the original TOC title (with ID) always
            display_title = title

            # build a separate name for the filename (ID stripped if requested)
